...
class BaseDialog {
    ...
    constructor(context: Context?) {
        builder = AlertDialog.Builder(context).setOnDismissListener { initialize() }
    }
    ...
    constructor(
        ...
        builder = AlertDialog.Builder(context)
            .setTitle(title)
            .setMessage(message)
            .setPositiveButton(positive) { dialog1: DialogInterface, _: Int ->
                dialog1.cancel()
                onPositiveClick?.onClick()
            }
            .setNegativeButton(negative) { dialog1: DialogInterface, _: Int ->
                dialog1.cancel()
    ...
    fun show() {
        if (!isShown) {
            ...
            dialog.show()
            forceShown()
            val textSize = dialog.context.resources.getDimension(R.dimen._12sp)
